...
func (tq *TrackQuery) QueryPlaylists() *PlaylistQuery {
	query := (&PlaylistClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, track.PlaylistsTable, track.PlaylistsPrimaryKey...),
		)
...
func (tq *TrackQuery) Clone() *TrackQuery {
	...
	return &TrackQuery{
		...
		withPlaylists: tq.withPlaylists.Clone(),
...
func (tq *TrackQuery) WithPlaylists(opts ...func(*PlaylistQuery)) *TrackQuery {
	query := (&PlaylistClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withPlaylists = query
	return tq
}
...
func (tq *TrackQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Track, error) {
	...
	if query := tq.withPlaylists; query != nil {
		if err := tq.loadPlaylists(ctx, query, nodes,
			func(n *Track) { n.Edges.Playlists = []*Playlist{} },
			func(n *Track, e *Playlist) { n.Edges.Playlists = append(n.Edges.Playlists, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TrackQuery) loadPlaylists(ctx context.Context, query *PlaylistQuery, nodes []*Track, init func(*Track), assign func(*Track, *Playlist)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(track.PlaylistsTable)
		s.Join(joinT).On(s.C(playlist.FieldID), joinT.C(track.PlaylistsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(track.PlaylistsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(track.PlaylistsPrimaryKey[1]))
