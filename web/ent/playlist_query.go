...
func (pq *PlaylistQuery) QueryTracks() *TrackQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, playlist.TracksTable, playlist.TracksPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *PlaylistQuery) WithTracks(opts ...func(*TrackQuery)) *PlaylistQuery {
	query := (&TrackClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PlaylistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Playlist, error) {
	...
	if query := pq.withTracks; query != nil {
		if err := pq.loadTracks(ctx, query, nodes,
			func(n *Playlist) { n.Edges.Tracks = []*Track{} },
			func(n *Playlist, e *Track) { n.Edges.Tracks = append(n.Edges.Tracks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlaylistQuery) loadTracks(ctx context.Context, query *TrackQuery, nodes []*Playlist, init func(*Playlist), assign func(*Playlist, *Track)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(playlist.TracksTable)
		s.Join(joinT).On(s.C(track.FieldID), joinT.C(playlist.TracksPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(playlist.TracksPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(playlist.TracksPrimaryKey[0]))
